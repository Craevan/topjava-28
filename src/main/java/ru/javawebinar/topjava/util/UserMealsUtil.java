...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> dailyCaloriesCount = new HashMap<>();
        for (UserMeal meal : meals) {
            dailyCaloriesCount.merge(meal.getDateTime().toLocalDate(), meal.getCalories(), Integer::sum);
        }
        ...
        for (UserMeal meal : meals) {
            if (TimeUtil.isBetweenHalfOpen(meal.getDateTime().toLocalTime(), startTime, endTime)) {
                result.add(new UserMealWithExcess(meal.getDateTime(),
                        meal.getDescription(),
                        meal.getCalories(),
                        dailyCaloriesCount.get(meal.getDateTime().toLocalDate()) > caloriesPerDay));
            }
        }
        return result;
    }
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> dailyCaloriesCount = meals.stream()
        ...
        return meals.stream()
                ...
                .map(userMeal -> new UserMealWithExcess(
                        ...
                        dailyCaloriesCount.get(userMeal.getDateTime().toLocalDate()) > caloriesPerDay))
